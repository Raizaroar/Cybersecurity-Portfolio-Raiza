# Cybersecurity-Portafolio-Raiza

## Cybersecurity Portfolio

[![GitHub](https://img.shields.io/badge/GitHub-Portfolio-blue?style=for-the-badge&logo=github)](tu-link)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin)]((https://github.com/Raizaroar/Cybersecurity-Portafolio-Raiza.git))

## About Me

Cybersecurity Analyst specializing in  **Network Traffic**. This repository showcases practical labs, security tools, and real-world projects developed during my professional transition into cybersecurity.

Skills & Technologies

``` Penetration Testing  | Metasploit | Burp Suite | Nmap
Security Operations  | SIEM | Splunk | Suricata
 Programming         | Python | Bash | PowerShell
Security Frameworks | OWASP | MITRE ATT&CK | NIST
```

## Repository Structure

| Directory | Description | Key Technologies |
|-----------|-------------|------------------|
| [01-Network-Security](./Labs/01-Network-Security) | Network scanning, packet analysis | Nmap, Wireshark |
| [02-Web-Application-Security](./Labs/02-Web-Application-Security) | Web vulnerabilities testing | Burp Suite, OWASP ZAP |
| [03-Penetration-Testing](./Labs/03-Penetration-Testing) | Exploitation frameworks & custom tools | Metasploit, Python |
| [04-SIEM-Projects](./Labs/04-SIEM-Projects) | Security monitoring & log analysis | Splunk, ELK |
| [05-Forensics](./Labs/05-Forensics) | Digital forensics investigations | Volatility, Autopsy |
| [06-Automation-Scripts](./Labs/06-Automation-Scripts) | Security automation tools | Python, Bash |
| [07-Threat-Intelligence](./Labs/07-Threat-Intelligence) | IOC analysis & threat hunting | MISP, OpenCTI |
| [08-Incident-Response](./Labs/08-Incident-Response) | IR playbooks & procedures | NIST Framework |

## Featured Projects

## [Advanced Port Scanner with Vulnerability Detection](./01-Network-Security/nmap-automation)

- Automated network reconnaissance tool
- CVE identification integration
- **Tech:** Python, Nmap, SQLite

### [Custom SIEM Dashboard for Threat Detection](./04-SIEM-Projects/splunk-dashboards)

- Real-time security monitoring
- Automated alert correlation
- **Tech:** Splunk, SPL, Python

### SQL Injection Testing Framework](./02-Web-Application-Security/sql-injection-labs)

- Automated SQLi vulnerability scanner
- Custom payload generation
- **Tech:** Python, Requests, BeautifulSoup

## Certifications

- **[CCST Cybersecurity Mujer Digital - Junior Achievement Mexico City]** - [2025]
- **[Google Cybersecurity Professional Certificate]** - [2025]
- **[IBM Cybersecurity Specialist Professional Certificate]** - [2025]
- **[AWS Certified Cloud Practitioner]** - [2025]
-**[Git and Github Essential]** - [2025]

## Contact

- **Email:** <raiza.rosas.ag1@outlook.com>
- **LinkedIn:** [Raiza Rosas](www.linkedin.com/in/raiza-rosas98)
- **GitHub:** [Raizaroar](https://github.com/Raizaroar)

---

⚠️ **Disclaimer:** All projects are conducted in controlled lab environments for educational purposes only.
